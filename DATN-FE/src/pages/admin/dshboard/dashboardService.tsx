...
import { APIResponse, DailyStats, RevenueResponse, TopSellingResponse } from './type';
  ...
  try {
    ...
    if (type === 'day' && revenueData.Ngày?.Doanh_thu) {
      dailyStats = revenueData.Ngày.Doanh_thu.map(item => ({
        ...
        revenueGrowth: item.growth_percentage === "Không có dữ liệu ngày trước" ? 0 : parseFloat(item.growth_percentage),
        orders: 0,
        users: 0,
        ordersGrowth: 0,
        usersGrowth: 0
