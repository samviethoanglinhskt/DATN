...
//     try {
//       setLoading(true);
//       const [productRes, categoriesRes, brandsRes, sizesRes, colorsRes] = await Promise.all([
...
//         tb_category_id: productRes.data.tb_category_id,
//         tb_brand_id: productRes.data.tb_brand_id,
...
//     if (currentFile) {
//       const isLessThan5MB = currentFile.size / 1024 / 1024 < 5;
//       if (!isLessThan5MB) {
//         message.error('Image must be smaller than 5MB');
//         return;
//       }

//       const isValidType = currentFile.type.startsWith('image/');
//       if (!isValidType) {
...
//       formData.append('tb_category_id', values.tb_category_id.toString());
//       formData.append('tb_brand_id', values.tb_brand_id.toString());
...
//     const variantImages = variant.images?.map((img: any, index: number) => ({
//       uid: `-${index}`,
//       name: `Image ${index + 1}`,
//       status: 'done',
//       url: typeof img === 'string' ? `http://127.0.0.1:8000/${img}` : `http://127.0.0.1:8000/${img.url}`,
//     })) || [];
...
//       const formData = new FormData();
//       formData.append('tb_size_id', values.tb_size_id.toString());
//       formData.append('tb_color_id', values.tb_color_id.toString());
...
//       // Handle images
//       if (values.images) {
//         const imageFiles = values.images.fileList || values.images;
//         imageFiles.forEach((file: any, index: number) => {
//           if (file.originFileObj) {
//             formData.append(`images[]`, file.originFileObj);
//           } else if (file.url) {
//             // Extract image path from full URL
//             const imagePath = file.url.replace('http://127.0.0.1:8000/', '');
...
//                     <div>
//                       <p>Size: {sizes.find((s) => s.id === variant.tb_size_id)?.name}</p>
//                       <p>Color: {colors.find((c) => c.id === variant.tb_color_id)?.name}</p>
//                       <p>SKU: {variant.sku}</p>
...
//                           <img
//                             key={index}
//                             src={`http://127.0.0.1:8000/${typeof image === 'string' ? image : image.url}`}
//                             alt={`Variant ${index}`}
...
//               </Button>
//             ) : (
//               <Button type="primary" onClick={handleProductUpdate} loading={loading}>
//                 Update Product
//               </Button>
//             )}
//           </Space>
...
//                 const { file } = info;
//                 if (file.originFileObj) {
//                   const isLessThan5MB = file.originFileObj.size / 1024 / 1024 < 5;
//                   if (!isLessThan5MB) {
//                     message.error('Image must be smaller than 5MB');
//                     return;
//                   }

//                   const isValidType = file.originFileObj.type.startsWith('image/');
//                   if (!isValidType) {
//                     message.error('You can only upload image files');
//                     return;
//                   }
...
//               }}
//             >
//               {(variantForm.getFieldValue('images')?.length || 0) >= 4 ? null : (
//                 <div>
//                   <PlusOutlined />
//                   <div style={{ marginTop: 8 }}>Upload</div>
//                 </div>
...
const ProductEdit: React.FC = () => {
  ...
  const handleVariantUpdate = async () => {
    try {
      ...
      const formData = new FormData();
      formData.append("tb_size_id", values.tb_size_id);
      formData.append("tb_color_id", values.tb_color_id);
      formData.append("sku", values.sku);
